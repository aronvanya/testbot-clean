...
def webhook():
    ...
    if data and "message" in data:
        ...
        thread_id = message.get("message_thread_id")  # ⬅️ поддержка темы
        ...
        if 'instagram.com/reel/' in text:
            ...
            processing_message_id = send_message(chat_id, "⏳ Обрабатываю ссылку, подождите...", thread_id=thread_id)
            success = send_reels_video(chat_id, text.strip(), user_name, thread_id)
            ...
            if success:
                ...
                send_message(chat_id, "❌ Не удалось скачать видео. Проверьте ссылку.", thread_id=thread_id)
...
def send_message(chat_id, text, parse_mode=None, thread_id=None):
...
def send_reels_video(chat_id, reels_url, user_name, thread_id=None):
    try:
        ...
        if video_url:
            ...
            if video_size_mb > MAX_DOC_SIZE_MB:
                send_message(chat_id, "❌ Видео слишком большое (более 2 ГБ). Telegram не поддерживает такие файлы.", thread_id=thread_id)
                return False
            elif video_size_mb > MAX_VIDEO_SIZE_MB:
                send_video_as_document(chat_id, video_content, user_name, thread_id)
            else:
                send_video_as_stream(chat_id, video_content, user_name, thread_id)
            return True
        else:
...
def send_video_as_stream(chat_id, video_content, user_name, thread_id=None):
...
def send_video_as_document(chat_id, video_content, user_name, thread_id=None):
